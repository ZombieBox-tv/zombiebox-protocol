...
with tempfile.TemporaryDirectory() as tmp:
    ...
    environment = {**os.environ, "GOMAXPROCS": "2", "ZOMBIE_CONTRACT_CAPTURE": str(capture)}
    subprocess.run(["go", "test", "-p", "2", "./internal/server", "-run", "^TestWireContracts$", "-count=1"], cwd=root.parent / "gateway", env=environment, check=True)
    for name, sample in json.loads(capture.read_text()).items():
        Draft202012Validator({"$defs": schema["$defs"], "$ref": f"#/$defs/{name}"}).validate(sample)
